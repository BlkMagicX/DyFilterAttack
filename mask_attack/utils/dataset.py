...
def custom_collate_fn(batch):
    ...
    image = torch.stack([sample['image'] for sample in batch], dim=0).to(device)     # (B, C, H, W)
    mask = torch.stack([sample['mask'] for sample in batch], dim=0).to(device)       # (B, C, H, W)
    batch_indices = torch.stack([sample['batch_indices'] for sample in batch], dim=0).to(device)
    classes = torch.stack([sample['classes'] for sample in batch], dim=0).to(device)
    bboxes = torch.stack([sample['bboxes'] for sample in batch], dim=0).to(device)
    
    # 4. 执行填充操作
    label = pad_sequence(label, batch_first=True, padding_value=-1).to(device)                   # (B, N_max, D)
    batch_indices = pad_sequence(batch_indices, batch_first=True, padding_value=-1).to(device)     # (B, N_max, 1)
    classes = pad_sequence(classes, batch_first=True, padding_value=-1).to(device)                 # (B, N_max, 1)
    bboxes = pad_sequence(bboxes, batch_first=True, padding_value=-1).squeeze(1).to(device)                   # (B, N_max, 4)
...
def create_temp_yaml(train_path, val_path, origin_yaml_path):
    # 读取原始YAML文件内容
