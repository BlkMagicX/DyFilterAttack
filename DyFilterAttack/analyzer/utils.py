...
class FilterAnalyzer:
    ...
    def visualize_opt(
        self, target_layer_path, size=56, upscaling_steps=12, upscaling_factor=1.2, filter_idx=0, lr=0.1, opt_steps=20, blur=None, cmd=False
    ) -> None:
        loss = 0
        sz = size
    ...
    def multi_img_single_layer_mean_activations(self, target_layer_path, left_filter_idx, right_filter_idx, show=False, size=640):
        def process_folder(sample_num, folder):
            ...
            for img_path in sampled_img_paths:
                mean_act = self.compute_single_layer_mean_activation(img_path, target_layer_path, left_filter_idx, right_filter_idx, size)
        ...
        def plot_multi_img_single_layer_mean_activations():
            # 创建双子图布局
            fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 6), gridspec_kw={'height_ratios': [2, 1]})  # 主图占2份高度，差值图占1份

            # 定义颜色和线型方案
            colors = ['b', 'r']
            linestyle_dict = {self.origin_img_dir: '--', self.attack_img_dir: '-'}
            ...
            ax1.legend(handles=legend_handles, labels=['origin', 'attack'], loc="upper right", ncol=1, fontsize=8)
            # 绘制差值图
            ax2.plot(np.arange(len(self.diff)), self.diff, color='g', linestyle='-', linewidth=2, label='Difference (Group1 - Group2)')
            ...
            legend_elements = [
                ...
                plt.Line2D(
                    ...
                    marker='o',
                    color='w',
                    label=f'Significant Points (n={len(exceeding_indices)})',
                    markerfacecolor='r',
                    markersize=10,
                    markeredgecolor='k',
                ),
        ...
        if show:
            plot_multi_img_single_layer_mean_activations()
        if not show:
            print(f"{target_layer_path}: index of outlier value: {exceeding_indices}")
    ...
    def analyze_layers(self, layer_configs, show=False):
        ...
        for config in layer_configs:
            ...
            result = self.multi_img_single_layer_mean_activations(
                target_layer_path=target_layer_path, left_filter_idx=left, right_filter_idx=right, show=show
            )
...
def analysis_all_layer(model, origin_img_dir, attack_img_dir, show):
    ...
    for config in LayerConfig:
        ...
        try:
            ...
            continue
        # 创建分析器并执行分析
        analyzer = FilterAnalyzer(model=model, module=module, parent_path=parent_path, origin_img_dir=origin_img_dir, attack_img_dir=attack_img_dir)
...
def analysis_all_layer_cross_classes(yolo, sample_classes=['0', '2', '14', '23', '39']):
    ...
    print(
        '''The following are the intersections of critical convolution kernel indices across categories.\n'''
